...
This directory owns the machine-readable contracts used by the primary guided
deployment experience, [Ranch Hand Public Preview](https://wranglerlabs.org/ranch-hand),
and by user-owned automation. RepoWrangler remains deployable with a clone,
fork, or an operator's own tooling.
...
Operators should normally consume these payloads through the separate
[Ranch Hand Windows application](https://wranglerlabs.org/ranch-hand), or may
...
refuses a tag that does not match `package.json`. The GHCR package must be
public: after publishing the exact digest, the workflow obtains a registry token
without GitHub credentials and fetches that manifest. Publication stops before
release assets are created if a clean Ranch Hand or Compose host would receive
